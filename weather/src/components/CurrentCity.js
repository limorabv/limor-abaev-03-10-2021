...
const CurrenCity = () => {
    ...
    const fetchCurrWeather = async (cityId) => {
        ...
        try { 
            ...
            dispatch (currentCityActions.updateCurrentWeather({
                ...
                imperial: currentWeatherJson[0].Temperature.Imperial, 
                metric: currentWeatherJson[0].Temperature.Metric
        }));
        } catch (error) {
    ...
    <Fragment>
        ...
        <Card className = 'city'>
            ...
            {currentWeather && <h2>{currentWeather.temperature.imperial.Value}<span>&#176;F</span></h2>}
